...
| 56 | Invariante "objeto único" de `operacao_ativos` garantida por **trigger**, não por índice UNIQUE parcial | A condição "operação não-terminal" depende de `operacoes.status` — outra tabela. Índice parcial do Postgres não aceita subconsulta/predicado cruzando tabelas, então um `UNIQUE … WHERE` não consegue exprimir a regra (e um UNIQUE global em `ativo_id WHERE papel='objeto'` seria errado: um ativo é objeto de muitas operações ao longo da vida). O trigger (`BEFORE INSERT/UPDATE`) checa as outras operações não-terminais e barra com `unique_violation`. Fiel ao doc 03 §3 ("o banco garante, não a disciplina"); o service segue como 1ª linha (status do ativo) |
| 57 | CORS **fechado por padrão** (`origin:false`), liberável por `CORS_ORIGINS` | Front e API são **same-origin** (Caddy em produção, proxy do Vite em dev) — não há chamada cross-origin legítima. Registrar `@fastify/cors` com `origin:false` não emite `Access-Control-Allow-Origin`, então o navegador bloqueia qualquer origem cruzada; um operador pode liberar origens específicas via env sem mexer no código. Postura restritiva explícita e configurável, sem mudar o comportamento atual |
| 55 | Copiloto Fase 2 estreia com **uma** ferramenta de escrita não-destrutiva: `propor_lancamento`, que **propõe** (não cria). A criação real só ocorre quando o humano confirma na UI, disparando `POST /lancamentos` com a própria autoria | Concretiza a decisão #43 com o menor risco: criar um lançamento `previsto` é reversível (cancelar/anular) e não mexe em máquina de estados. A ferramenta de proposta vive **fora** de `FERRAMENTAS_LEITURA` (o invariante "leitura não muta" segue testável) e só é exposta a quem tem `lancamentos:criar` (revalidado dentro da ferramenta). O modelo nunca escreve: a proposta é inerte (teste conta linhas antes/depois). Ações destrutivas (anular/estornar/transicionar/excluir) ficam **fora** — entram só com demanda e guardrails próprios. Conta/categoria são escolhidas pelo humano ao confirmar, então a IA não precisa adivinhar UUIDs |

## Sprint 10 — Repaginação de UI e interligação

| # | Decisão | Por quê |
|---|---------|---------|
| 58 | Guard anti-duplicação (porta de entrada única): quem entra por "novo ativo" cria a compra (que não recria o ativo); quem entra por "nova operação de compra" cria o ativo (que não dispara outra compra); idem lançamento. Substitui conscientemente a decisão do Sprint 5 "compra vincula ativo existente" | A cadeia A→B→A causaria loop infinito e dados duplicados. A porta de entrada define o fluxo; os opcionais de interligação (campos aparecem só quando marcado) são de mão única. Docx: se você entrou por "novo ativo", a compra criada internamente NÃO gera outro ativo. |
...
## Sprint 16 — Um só caminho

| # | Decisão | Por quê |
|---|---------|---------|
| 69 | **A URL é a fonte da verdade do estado de tela** (aba e filtro), por dois hooks em `hooks/estadoUrl.ts`; o valor é derivado de `useSearchParams` a cada render, nunca copiado para `useState` | Copiar para estado local lê a URL só na montagem: voltar/avançar no navegador mudava o endereço e não a tela. Derivar resolve os dois sentidos de graça e torna toda tela linkável. Escrita sempre com `{replace:true}` — trocar de aba não deve empilhar histórico |
| 70 | **Um destino de topo por assunto**: Financeiro, Dashboard $ e Relatórios viram cinco abas de `/financeiro`; as rotas antigas redirecionam | Eram três telas para "ver o mesmo dinheiro de ângulos diferentes", e o usuário precisava memorizar qual ângulo morava em qual item de menu. Redirect (padrão do `/guinchos`, Sprint 5) preserva bookmarks sem manter tela paralela |
| 71 | No hub, a aba de Lançamentos mantém os **params legados sem prefixo**; as demais usam prefixo próprio (`p_`, `pl_`, `dre_`) | `status`, `tipo` e `ano` têm significados diferentes em Lançamentos e na Planilha — sem namespace, um deep-link de uma aba produziria estado inválido na outra. Os nomes sem prefixo são contrato público: sete deep-links das fichas e do dashboard dependem deles e não podiam quebrar |
| 72 | Navegação declarada como **dado** em `componentes/navegacao.ts` (permissão, item primário e tecla de atalho no próprio item) | A lista vivia dentro do Layout com permissão por spread condicional, e a barra inferior derivava de paths escritos à mão — dava para um item existir num menu e faltar no outro. Como dado, as três superfícies e os atalhos leem da mesma fonte |
| 73 | Permissão também **na rota** (`Exige`), não só no menu | Até aqui o `pode(...)` apenas escondia o item: quem digitasse `/usuarios` renderizava a tela e só a API barrava os dados. Defesa em profundidade — a mesma lógica vale por aba dentro do hub |
| 74 | **Um único dono do `keydown` global** (`ProvedorAtalhos`), que absorve o ⌘K antes preso dentro do BuscaGlobal | Dois listeners disputando as mesmas teclas dá comportamento imprevisível, e o `/` precisava do estado "paleta aberta" que era local à busca. Teclas nuas são ignoradas quando o foco está num campo — senão digitar "g" num filtro navegaria |
| 75 | Copiloto Fase 3 (`propor_operacao`) mantém o guardrail da #43 e **não reconstrói o formulário de operação**: grava o rascunho que `/operacoes/nova` já sabe restaurar e leva o usuário para lá | Operação tem quatro fluxos, validação visível e regras de desconto/caução que já existem numa tela. Recriar isso no drawer seria duplicar regra de negócio — o copiloto adianta a digitação, quem cria é o humano no lugar de sempre |
| 76 | Ferramentas de proposta em **listas separadas por permissão** (`FERRAMENTAS_PROPOSTA` para `lancamentos:criar`, `FERRAMENTAS_PROPOSTA_OPERACAO` para `operacoes:criar`) | O invariante testável "a lista de leitura não tem nenhum verbo de mutação" continua valendo, e cada proposta é exposta exatamente a quem pode executá-la. Ids fora do formato uuid são descartados: se o modelo inventar um id, o campo fica vazio para o humano preencher |
| 77 | **Versão do formato do índice de busca no código** (`BUSCA_INDICE_VERSAO`) + marca em `meta_sistema`; divergiu, a API reindexa no arranque | O índice é derivado, então reconstruir é sempre seguro — o que faltava era gatilho. Depender de alguém lembrar de rodar `pnpm busca:reindexar` no VPS deixou o índice desatualizado em produção por um sprint inteiro. `meta_sistema` guarda estado do sistema, não dado de negócio |
| 78 | Tipos de manutenção padrão são **imutáveis** (não renomeáveis nem desativáveis) | `garantirTiposManutencaoPadrao()` os recria por nome a cada arranque; permitir rename faria bootstrap e usuário brigarem para sempre. Desativação (em vez de exclusão) tira do seletor sem apagar o histórico de quem já usa o tipo |
