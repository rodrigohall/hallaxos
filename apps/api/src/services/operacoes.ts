...
// ── Helpers financeiros: operação gera lançamentos previstos (doc 03 §4) ──
async function contaPadrao(tx: DbConn): Promise<string> {
...
async function categoriaPadrao(tx: DbConn, nome: string, tipo: "receita" | "despesa"): Promise<string> {
...
/** Gera lançamentos previstos vinculados à operação (com parcelas mensais). */
async function gerarLancamentos(
  ...
  for (let i = 0; i < n; i++) {
    ...
    await registrarEvento(tx, {
      entidadeTipo: "operacao", entidadeId: o.operacaoId, evento: "lancamento_gerado",
